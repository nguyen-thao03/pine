...
class OrderModel {
  ...
  factory OrderModel.fromSnapshot(DocumentSnapshot snapshot) {
    ...
    return OrderModel(
      ...
      status: data.containsKey('status') ? OrderStatus.values.firstWhere((e) => e.name == data['status'], orElse: () => OrderStatus.pending,): OrderStatus.pending,
      totalAmount: data.containsKey('totalAmount') ? (data['totalAmount'] as num).toDouble() : 0,
      ...
      address: data.containsKey('address') ? AddressModel.fromMap(data['address'] as Map<String, dynamic>) : AddressModel.empty(),
      ...
      shippingCost: data.containsKey('shippingCost') ? (data['shippingCost'] as num).toDouble() : 0,
    );
  }
}